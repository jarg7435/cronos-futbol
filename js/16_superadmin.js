...
window.saClubs = async function saClubs() {
    ...
    try {
        ...
        const renderRow = (u, cid) => {
            ...
            const em   = (u.email||u.id).replace(/'/g,"\\'");
            return `
            <div style="display:flex;align-items:center;gap:0.4rem;padding:0.48rem 0.5rem;border-bottom:1px solid rgba(255,255,255,0.04);">
                ...
                <div style="flex:1;min-width:0;">
                    <div style="font-size:0.81rem;color:white;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${u.email||u.id}</div>
                ...
                <div style="display:flex;gap:0.2rem;flex-shrink:0;">
                    ${st==='pending'?`<button onclick="saQuickApprove('${u.id}','${em}','${cid}')" title="Aprobar (SA)" style="padding:0.22rem 0.45rem;background:rgba(255,215,0,0.15);border:1px solid rgba(255,215,0,0.4);border-radius:5px;color:#ffd700;font-size:0.68rem;cursor:pointer;font-weight:700;">✅ SA</button>`:''}
                    ${st==='active'?`<button onclick="saSetClubUserStatus('${u.id}','${em}','blocked','${cid}')" style="padding:0.22rem 0.45rem;background:rgba(240,136,62,0.15);border:1px solid rgba(240,136,62,0.4);border-radius:5px;color:#f0883e;font-size:0.68rem;cursor:pointer;">🔒</button>`:''}
                    ${st==='blocked'?`<button onclick="saSetClubUserStatus('${u.id}','${em}','active','${cid}')" style="padding:0.22rem 0.45rem;background:rgba(63,185,80,0.15);border:1px solid rgba(63,185,80,0.4);border-radius:5px;color:#3fb950;font-size:0.68rem;cursor:pointer;">✅</button>`:''}
                    ${st!=='removed'?`<button onclick="saSetClubUserStatus('${u.id}','${em}','removed','${cid}')" style="padding:0.22rem 0.45rem;background:rgba(255,88,88,0.15);border:1px solid rgba(255,88,88,0.4);border-radius:5px;color:#ff5858;font-size:0.68rem;cursor:pointer;">🗑️</button>`:''}
                </div>
            </div>`;
        };
        ...
        Object.values(clubs).forEach(c => {
            ...
            <div style="margin-bottom:1rem;border:1px solid rgba(255,255,255,0.08);border-radius:10px;overflow:hidden;">
                <div style="background:rgba(88,166,255,0.07);padding:0.6rem 0.9rem;display:flex;justify-content:space-between;align-items:center;">
                    <span style="font-weight:700;color:white;font-size:0.9rem;">🏟️ ${c.name||c.id}</span>
                    <div style="display:flex;gap:0.5rem;align-items:center;">
                        ...
                        <button onclick="openClubAdminPanel('${c.id}')" style="padding:0.22rem 0.5rem;background:rgba(88,166,255,0.12);border:1px solid rgba(88,166,255,0.3);border-radius:5px;color:#58a6ff;font-size:0.68rem;cursor:pointer;font-weight:700;">⚙️ Gestionar</button>
                    </div>
                </div>
    ...
    try {
        ...
        const buildCard = (item, srcType) => {
            ...
            if (item.playerNumber) {
                extraRows += `<div><div style="color:#8b949e;font-size:0.67rem;">Dorsal jugador</div><div style="color:white;">#${item.playerNumber}${item.playerAlias?' · '+item.playerAlias:''}</div></div>`;
            }
            ...
            <div style="background:rgba(255,255,255,0.04);border:1px solid rgba(${borderRGB},0.25);border-radius:9px;padding:0.85rem;margin-bottom:0.6rem;">
                ...
                <div style="background:rgba(255,255,255,0.03);padding:0.55rem 0.65rem;border-radius:7px;margin-bottom:0.6rem;display:grid;grid-template-columns:1fr 1fr;gap:0.35rem;font-size:0.82rem;">
                    <div><div style="color:#8b949e;font-size:0.67rem;">Email</div><div style="color:white;font-weight:600;word-break:break-all;">${email||'–'}</div></div>
                    <div><div style="color:#8b949e;font-size:0.67rem;">Club</div><div style="color:white;font-weight:600;">${club}</div></div>
                    ${name&&name!=='–'?`<div style="grid-column:1/-1;"><div style="color:#8b949e;font-size:0.67rem;">Nombre</div><div style="color:white;">${name}</div></div>`:''}
                    ${extraRows}
                </div>
        ...
        if (quotaReqs.length) {
            ...
            ${quotaReqs.map(r => {
                ...
                <div style="background:rgba(255,255,255,0.04);border:1px solid rgba(240,136,62,0.25);border-radius:9px;padding:0.85rem;margin-bottom:0.6rem;">
                    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:0.6rem;">
                        <div>
                            <div style="font-weight:700;color:#f0883e;font-size:0.88rem;">${r.clubName||r.clubId||'–'}</div>
...
window.saTrash = async function saTrash() {
    ...
    try {
        ...
        const row = (u, brgb, btns) => `<div style="background:rgba(255,255,255,0.04);border:1px solid rgba(${brgb},0.3);border-radius:8px;padding:0.62rem 0.72rem;display:flex;justify-content:space-between;align-items:center;margin-bottom:0.35rem;"><div><div style="font-weight:700;color:white;font-size:0.82rem;">${u.email||u.id}</div><div style="font-size:0.69rem;color:#8b949e;">${window.ROLE_META[u.role]?.label||u.role||'?'} · ${u.clubName||''} · ${u.status==='removed'?'Baja: '+fmt(u.removedAt):'Bloq: '+fmt(u.blockedAt)}${u.authDeleted?' · <span style="color:#3fb950">✅ Auth limpio</span>':''}</div></div><div style="display:flex;gap:0.25rem;flex-shrink:0;">${btns}</div></div>`;
        ...
        if (removed.length) {
            ...
            html += removed.map(u=>row(u,'255,88,88',`<button onclick="saPurgeUser('${u.id}','${(u.email||u.id).replace(/'/g,"\\'")}' )" style="padding:0.28rem 0.58rem;background:rgba(255,88,88,0.15);border:1px solid rgba(255,88,88,0.4);border-radius:5px;color:#ff5858;font-size:0.7rem;cursor:pointer;font-weight:700;">🗑️ Limpiar</button>`)).join('');
        ...
        if (blocked.length) {
            ...
            html += blocked.map(u=>row(u,'240,136,62',`<button onclick="saSetClubUserStatus('${u.id}','${(u.email||u.id).replace(/'/g,"\\'")}','active','${u.clubId||''}')" style="padding:0.28rem 0.58rem;background:rgba(63,185,80,0.15);border:1px solid rgba(63,185,80,0.4);border-radius:5px;color:#3fb950;font-size:0.7rem;cursor:pointer;font-weight:700;">✅</button><button onclick="saPurgeUser('${u.id}','${(u.email||u.id).replace(/'/g,"\\'")}' )" style="padding:0.28rem 0.58rem;background:rgba(255,88,88,0.15);border:1px solid rgba(255,88,88,0.4);border-radius:5px;color:#ff5858;font-size:0.7rem;cursor:pointer;font-weight:700;">🗑️</button>`)).join('');
        }
        body.innerHTML = html;
    } catch (e) {
        body.innerHTML = `<p style="color:#ff5858;text-align:center;padding:2rem;">⚠️ ${e.message}</p>`;
...
console.log('✅ 16_superadmin.js v9.0 cargado');
