...
window.saSecretary = async function saSecretary() {
    ...
    <div style="max-width:600px;">
        ...
        <p style="font-size:0.8rem;color:#8b949e;margin:0 0 1.2rem;">
            Envía correos de invitación a usuarios para que completen su registro en la plataforma.
        </p>
        <div style="display:flex;flex-direction:column;gap:0.8rem;">
            <div>
                <label style="font-size:0.78rem;color:#8b949e;display:block;margin-bottom:4px;">Email de destino *</label>
                <input id="sec-email" type="email" placeholder="usuario@email.com"
            ...
            <div>
                ...
                <input id="sec-club" type="text" placeholder="Nombre del club si aplica"
                    style="width:100%;padding:0.7rem;background:rgba(255,255,255,0.05);
                           border:1px solid rgba(255,255,255,0.15);border-radius:8px;
                           color:white;font-size:0.9rem;box-sizing:border-box;">
            </div>
            ...
            <div>
                <label style="font-size:0.78rem;color:#8b949e;display:block;margin-bottom:4px;">Mensaje personalizado (opcional)</label>
                <textarea id="sec-body" rows="4" placeholder="Dejar vacío para usar el mensaje por defecto..."
                    style="width:100%;padding:0.7rem;background:rgba(255,255,255,0.05);
                           border:1px solid rgba(255,255,255,0.15);border-radius:8px;
                           color:white;font-size:0.9rem;box-sizing:border-box;resize:vertical;font-family:Inter,sans-serif;"></textarea>
            </div>
            <button onclick="saSendInviteEmail()"
...
// Enviar email de invitación vía Cloud Function
window.saSendInviteEmail = async function() {
    ...
    _saShowSpinner('Enviando invitación...');
    try {
        ...
        if (result.data?.success) {
            _saToast('✅ Invitación enviada a ' + to, 4000);
            document.getElementById('sec-email').value = '';
        } else {
            _saToast('⚠️ No se pudo enviar el email', 4000);
        }
    } catch (e) {
        _saHideSpinner();
